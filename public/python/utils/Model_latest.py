...
df_comb['symptoms_text'] = df_comb.iloc[:, 1:].apply(lambda row: ' '.join(row.index[row == 1]), axis=1)

# Fit TF-IDF vectorizer
vectorizer = TfidfVectorizer()
X_tfidf = vectorizer.fit_transform(df_comb['symptoms_text'])
...
x_train, x_test, y_train, y_test = train_test_split(X_tfidf, Y, test_size=0.10)
...
scores_mnb = cross_val_score(mnb, X_tfidf, Y, cv=5)
...
# performing cross validation with 5 different splits
scores_rf = cross_val_score(rf, X_tfidf, Y, cv=5)
# mean of cross val score (accuracy)
...
# performing cross validation with 5 different splits
scores_knn = cross_val_score(knn, X_tfidf, Y, cv=5)
# mean of cross val score (accuracy)
...
# performing cross validation with 5 different splits
scores_lr = cross_val_score(lr, X_tfidf, Y, cv=5)
# mean of cross val score (accuracy)
...
# performing cross validation with 5 different splits
scores_svm = cross_val_score(svm, X_tfidf, Y, cv=5)
# mean of cross val score (accuracy)
...
# performing cross validation with 5 different splits
scores_dt = cross_val_score(dt, X_tfidf, Y, cv=5)
# mean of cross val score (accuracy)
...
# performing cross validation with 5 different splits
scores_mlp = cross_val_score(mlp, X_tfidf, Y, cv=5)
# mean of cross val score (accuracy)
...
# Cross Validation Accuracy XGB
# performing cross validation with 5 different splits
scores_xgb = cross_val_score(xgb, X_tfidf, Y, cv=5)
# mean of cross val score (accuracy)
